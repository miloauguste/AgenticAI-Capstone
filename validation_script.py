...
class SystemValidator:
    ...
    def load_data(self) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        try:
            ...
            actual_df = pd.read_csv('generated_tickets.csv')
...
"""
    ...
    def run_validation(self) -> Dict:
        ...
        try:
            # Load data
            expected_df, actual_df = self.load_data()
            logger.info(f"Loaded {len(expected_df)} expected results and {len(actual_df)} actual results")
            
            # Validate classifications
            validation_results = self.validate_classifications(expected_df, actual_df)
            logger.info(f"Overall accuracy: {validation_results['accuracy']:.1f}%")
            
            # Generate confusion matrix
            confusion_matrix = self.generate_confusion_matrix(expected_df, actual_df)
            
            # Analyze confidence scores
            confidence_stats = self.analyze_confidence_scores(actual_df)
            
            # Generate and save report
            report = self.generate_report(validation_results, confidence_stats, confusion_matrix)
            ...
            self.save_detailed_results(validation_results)
            ...
            print(f"✅ Correct Classifications: {validation_results['correct_classifications']}")
            print(f"🎯 Overall Accuracy: {validation_results['accuracy']:.1f}%")
            
            if confidence_stats:
                print(f"📈 Average Confidence: {confidence_stats['mean']:.1f}%")
            
            print("\n📋 Category Accuracy:")
            for category, stats in validation_results['category_accuracy'].items():
                print(f"   {category}: {stats['accuracy']:.1f}%")
